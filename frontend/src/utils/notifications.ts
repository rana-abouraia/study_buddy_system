...
export type DedupedNotification<T extends NotificationLike> = T & {
  duplicateIds: string[];
  duplicateUnreadIds: string[];
};

const normalizeText = (value?: string | null) =>
  (value ?? '').trim().replace(/\s+/g, ' ').toLowerCase();

export const getNotificationTimeMs = (value: string) => {
...
const getNotificationKey = (notification: NotificationLike) =>
  [
...
export function dedupeNotifications<T extends NotificationLike>(
  notifications: T[] = []
): Array<DedupedNotification<T>> {
  const groups = new Map<string, DedupedNotification<T>>();
  ...
  return Array.from(groups.values()).sort(
    (a, b) => getNotificationTimeMs(b.createdAt) - getNotificationTimeMs(a.createdAt)
  );
}
...
export function replaceUserIdsWithNames(
  message: string,
  ...
  usersById.forEach((name, id) => {
    if (!id || !name || !displayMessage.includes(id)) return;
...
export function isSelfMatchNotification(
  ...
  if (!type.includes('match') && !title.includes('match')) return false;
  if (currentUser?.id && message.includes(normalizeText(currentUser.id))) return true;
  return Boolean(currentUserName && message.includes(currentUserName));
}

export function isConnectedMatchNotification(
  notification: NotificationLike,
  connectedUserIds: string[] = [],
  usersById: Map<string, { firstName?: string; lastName?: string }> = new Map()
) {
  const type = normalizeText(notification.type);
  const title = normalizeText(notification.title);
  const message = normalizeText(notification.message);
  ...
  return connectedUserIds.some((userId) => {
    ...
    return (
      message.includes(normalizeText(userId)) ||
